...
> 本文件是项目的"宪法"，规定了架构约定、命名规则、踩坑记录和开发流程。
...
1. **完整读完本文件**，不要跳读。
2. 确认你理解了"架构概览"和"铁律"两节。
3. 特别注意"血泪教训"——那些是真实踩过的坑，重复踩会被骂。
4. 如果你不确定某个改动是否符合规则，**先问用户，不要猜**。
...
1. **改代码前先读现有代码**，匹配现有的注释密度、命名风格、错误处理方式。Lumen 的 Rust 代码用中文注释，前端也是中文 UI 文案，保持一致。
2. **不要引入新依赖**，除非确实必要且用户同意。每加一个 crate / npm 包都要能说出理由。
3. **不要删除 .gitignore 里的条目**，尤其是 `*.db` 和 `target/`。
4. **改完一定要跑验证清单**（第 7 节），四项全绿才算完成。
5. **遇到不确定的设计决策，问用户，不要自作主张。** 本文档没覆盖的情况，按"最小惊讶原则"处理，并在完成后建议补充到本文档。
6. **本文档是活的**——踩了新坑、立了新规矩，要及时追加到第 6 节"血泪教训"或第 5 节"铁律"。

---

*最后更新：Phase 1+2 完成后。维护者：Summus。*
