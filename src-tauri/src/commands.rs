...
pub async fn add_memory(
    ...
    // Add the row synchronously, then embed if we can.
...
pub async fn chat(
    ...
    // 3) RAG: embed the user message and retrieve top-5 memories.
    ...
    // 4) Build the chat request with prior messages + system prompt.
    ...
    // 6) Background: extract facts from this turn (user msg + assistant reply).
    //    Failures are non-fatal — the chat already succeeded.
...
/// Return (role, content) pairs for a conversation, oldest first, excluding
/// any system messages (we synthesize the system prompt fresh each turn).
...
// Re-export so other modules can reach these without repeating paths.
